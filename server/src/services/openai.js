...
const SCOPE_RULE = `STRICT SCOPE RULE — You are exclusively a cloud infrastructure assistant. You ONLY answer questions about:
...
function buildSystemPrompt(context = {}) {
  const { topProvider, score, useCase, budget, priorities = [] } = context;
...
RESPONSE GUIDELINES:
- Plain English first: avoid jargon. When a technical term is unavoidable, define it in one clause.
...
async function generateExplanation(userAnswers, rankedProviders) {
  ...
  const systemPrompt = `You are CloudAdvisor, a plain-English cloud infrastructure expert. Write clear, friendly explanations for non-technical users. Never use bullet points in this output. No jargon without a brief definition. Be warm but precise.`;
